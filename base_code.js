...
//A boys
for(let i = 0; i < 100; i++){
    wiggly_men.push(new man(width*0.25 + spread*randn_bm(),200 + spread*randn_bm(), Math.random()*2*Math.PI, "rgb(255,0,0)", 20*Math.random(), (Math.random()>0.5)))
}

//B boys
for(let i = 0; i < 100; i++){
    wiggly_men.push(new man(width*0.75 + spread*randn_bm(),200 + spread*randn_bm(), Math.random()*2*Math.PI, "rgb(0,0,255)", 20*Math.random(), (Math.random()>0.5)))
}
...
function send_to_step_2(){
    if(wiggly_men[0].step != 1){
        ...
        for(let i = 0; i < wiggly_men.length; i++){
            ...
            var new_red = Math.floor((red1 + red2)/2)
            var new_blue = Math.floor((blue1 + blue2)/2)
